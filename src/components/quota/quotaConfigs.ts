...
const fetchCopilotQuota = async (
  ...
  for (const { key, id, labelKey } of COPILOT_QUOTA_CATEGORIES) {
    ...
    const remaining = detail.remaining ?? detail.quota_remaining ?? 0;
    const used = entitlement > 0 ? entitlement - remaining : 0;
    const usedPercent = entitlement > 0 ? Math.round((used / entitlement) * 100) : null;
  ...
  if (copilotPlan) {
    nodes.push(
      h(
        ...
        h('span', { className: styleMap.codexPlanValue }, copilotPlan)
      )
    );
  }
  ...
  if (resetDate) {
    ...
    nodes.push(
      h(
        ...
        h('span', { className: styleMap.codexPlanLabel }, t('copilot_quota.reset_label')),
